...
public class RoleFilter implements Filter {
    ...
    private static final String ROLE_PERMISSION = "role_permission.properties";

    public RoleFilter() throws Exception {
        properties = new Properties();
        properties.load(getClass().getClassLoader().getResourceAsStream(ROLE_PERMISSION));
        admin = new ArrayList<>(Arrays.asList(properties.getProperty("admin").split(",")));
        guest = new ArrayList<>(Arrays.asList(properties.getProperty("guest").split(",")));
        user = new ArrayList<>(Arrays.asList(properties.getProperty("user").split(",")));
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (uri.endsWith("jpg") || uri.endsWith("js") || uri.contains("css") || uri.endsWith("png") || uri.endsWith("ttf") || uri.endsWith("otf")
                ...
                || uri.endsWith("txt") || uri.endsWith("jpeg")) {
        ...
        if (role == null) {
            if (guest.contains(resource)) {
                url = properties.getProperties(resource);
            } else{
                req.setAttribute("VIEW_LIST", "viewlist");
            }
        } else if (role.equals("user")) { //user
            ...
            if (admin.contains(resource)) { //admin
                url = properties.getProperties(resource);
            }else{
                req.setAttribute("VIEW_LIST", "viewlist");
            }
